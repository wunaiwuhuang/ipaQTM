...
spqtm_cis <- list.files("../spqtm/cis_sig",pattern = "cisfilter_noex.txt$",full.names = T)
ipaqtm_cis <- list.files("../cis_sig",pattern = "cisfilter_noex.txt$",full.names = T)
spqtm_trans <- list.files("../spqtm/trans_sig",pattern = "transfilter_noex.txt$",full.names = T)
...
for (i in 1:length(ipaqtm_cis)) {
    ...
    spqtm <- fread(spqtm_cis[i]) %>% as.data.frame()
    
    # extract gene symbol from ipaqtm
    ipaqtm$symbol <- sapply(strsplit(ipaqtm$gene, ":"), function(x) x[2])
    # extract gene symbol from spqtm
    spqtm$symbol <- sapply(strsplit(spqtm$gene, ":"), function(x) x[2])

    #calculate overlap cg (in share genes)
    common_genes <- intersect(ipaqtm$symbol, spqtm$symbol)
    all_overlap_cg <- character(0)
    for (gene in common_genes) {
        ipa_cg_gene <- unique(ipaqtm$snps[ipaqtm$symbol == gene])
        spqtm_cg_gene <- unique(spqtm$snps[spqtm$symbol == gene])
        overlap_cg_gene <- intersect(ipa_cg_gene, spqtm_cg_gene)
...
for (i in 1:length(ipaqtm_trans)) {
    ...
    spqtm <- fread(spqtm_trans[i]) %>% as.data.frame()
...
# save the results
save(results, file = "./spqtm_overlap.RData")

load("./spqtm_overlap.RData")
# draw the figure
...
#save figure
ggsave(filename="./2.3.2.spqtm_overlap_cis.pdf",plot=p1,device="pdf",width=10,height=6,units="in",dpi=300)
ggsave(filename="./2.3.2.spqtm_overlap_trans.pdf",plot=p2,device="pdf",width=10,height=6,units="in",dpi=300)
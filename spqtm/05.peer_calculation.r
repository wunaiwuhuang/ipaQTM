...
# 要排除的癌种缩写
exclude_cancers <- c("BLCA", "BRCA", "COAD", "ESCA", "HNSC", "KIRP",
                     "LAML", "LGG", "LIHC", "LUAD", "LUSC", "PAAD",
                     "PRAD", "SARC", "SKCM", "TGCT", "THCA", "THYM")

# 过滤出不包含上述癌种缩写的文件
files <- files[!Reduce(`|`, lapply(exclude_cancers, function(x) grepl(x, files)))]

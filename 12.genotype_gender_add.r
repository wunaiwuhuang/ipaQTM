...
# select gender_considering cancers
self_clinical <- read.table("./00.all.info.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
self_clinical <- self_clinical %>% filter(!is.na(gender)) %>% group_by(Tumour) %>% filter((all(gender == 0) | all(gender == 1))) %>% ungroup() # not all 0/NA or 1/NA
cancers <- unique(self_clinical$Tumour)
for(cancer in cancers){
temp_clinic<-read.table(paste0("./data/",cancer,"_covariates.txt"), header = TRUE, sep = "\t",row.names = 1,check.names = FALSE)
temp_clinic<-temp_clinic[-1,]
write.table(temp_clinic, file = paste0("./data/",cancer,"_covariates_temp.txt"), sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
}
...
SNP_file_name <- paste0("./data/",cancer,"_mdnause.txt")
expression_file_name <- paste0("./data/",cancer,"_ipause.txt")
covariates_file_name <- paste0("./data/",cancer,"_covariates_temp.txt")
...
genepos = read.table(paste0("./data/",cancer,"_ipaloc.txt"),header=TRUE,stringsAsFactors=FALSE)
snpspos = read.table(paste0("./data/",cancer,"_mdnaloc.txt"),header=T,stringsAsFactors=FALSE)

output_file_name     = paste0("./trans_sig/",cancer,"_TransSig.txt") #trans
output_file_name_cis = paste0("./cis_sig/",cancer,"_CisSig.txt") #cis
...
pvOutputThreshold = 1e-6 #change this to 1e-6
useModel = useModel
...
{	
  ...
  cvrt = t( qr.Q(q) );
  peer_num = nrow(cvrt) 
  q.ethnicity <- t( qr.Q( qr(t(cvrt[c(1,3:7),])) ) ) 		# QR decomposition of intercept and 3 PCs
  q.ethnicity.PEER <- t( qr.Q( qr(t(cvrt[c(1,3:peer_num),])) ) ) # QR decomp of intercept + 3PCs + 15 PEER
  q.ethnicity.PEER.gender <- t( qr.Q( qr(t(cvrt[c(1,3:peer_num,2),])) ) ) # QR decomp of interc + 3PCs + 15 PEER + gender
  q.PEER <- t( qr.Q( qr(t(cvrt[c(1,8:peer_num),])) ) )
...
mclapply(cancers, process_cancer, mc.cores = 4)  # parallel processing
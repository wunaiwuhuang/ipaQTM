...
covariates_file_name <- paste0("./data/",cancer,"_covariates.txt")
...
pvOutputThreshold = 1e-6 #choose smaller p-value threshold
useModel = useModel
errorCovariance = errorCovariance
verbose = TRUE
output_file_name.cis = output_file_name_cis
pvOutputThreshold.cis = 0.05 #output all results
snpspos = snpspos
genepos = genepos
cisDist = 1e6
pvalue.hist = "qqplot"
...
{	
  ...
  if( cvrt$nRows()>0 ) {
    cvrt$SetNanRowMean(); #use this function to impute missing values
    cvrt$CombineInOneSlice();
    cvrt = rbind(matrix(1,1,snps$nCols()),cvrt$getSlice(1)); # add intercept
  } else {
  ...
  cvrt = t( qr.Q(q) );
  peer_num = nrow(cvrt)
  q.ethnicity <- t( qr.Q( qr(t(cvrt[c(1,4:8),])) ) ) 		# QR decomposition of intercept and 5 PCs
  q.ethnicity.PEER <- t( qr.Q( qr(t(cvrt[c(1,4:peer_num),])) ) ) # QR decomp of intercept + PCs +  PEER
  q.ethnicity.PEER.gender <- t( qr.Q( qr(t(cvrt[c(1,4:peer_num,2),])) ) ) # QR decomp of interc + PCs + PEER + gender
